DROP PROCEDURE IF EXISTS `checkSystemDesignPassed`;

DELIMITER $$

CREATE PROCEDURE `checkSystemDesignPassed` (
	IN userId INT(11),
...
BEGIN
    ...
    -- if user has not submitted system design
    ELSEIF NOT EXISTS
            (SELECT developer_id
            FROM developer_system_design
...
-- call checkSystemDesignPassed(158028, 1, null);